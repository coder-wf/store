								...
								<c:forEach items="${list }" var="o" varStatus="vs">
										<tr>
											...
											<td style="CURSOR: hand; HEIGHT: 22px" align="center"
												...
												<c:if test="${o.state == 1 }">
													<a href="${pageContext.request.contextPath}/adminOrderServlet?method=update&oid=${o.oid}">等待发货</a>
												</c:if>
												<c:if test="${o.state == 2 }">
													等待确认收货
												</c:if>
...
<script type="text/javascript">
	function findByOid(oid) {
		$.post("${pageContext.request.contextPath}/adminOrderServlet",{"method":"findByOid","oid":oid},
				function (result) {

					//解析result. 填充页面layer
					//alert("result="+result);

					var s = "<table border='1px' width='100%'>";

					//遍历result, 没遍历一次就是一行 tr
					$(result).each(function(i,obj){
						s+="<tr><td><img width='100px' height='80px' src='${pageContext.request.contextPath}/"+obj.product.pimage+"'/></td><td>"+obj.product.pname+"</td><td>"+obj.count+"</td></tr>";
					});
					s+="</table>";
